public class MyMain {
    ...
    public static boolean isPalindrome(String str) {
        return str.length() < 2 || (str.charAt(0) == str.charAt(str.length() - 1)) && isPalindrome(str.substring(1, str.length() - 1));
    }

    // This method returns the orignal string reversed;
    // this method should be written using recursion
    public static String reverse(String str) {
        return (str.length() < 2) ? str : str.substring(str.length()-1) + reverse(str.substring(1, str.length()-1)) + str.charAt(0);
    }
